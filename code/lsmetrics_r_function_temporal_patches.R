## function ----
lsmetrics_r_temporal_patches <- function(x1, x2, path) {

  # binary
  x2_2 <- terra::ifel(x2 == 1, 2, 0)

  # temporal
  x1_x2_temporal <- x1 + x2_2

  # temporal null
  x1_x2_temporal_null <- terra::ifel(x1_x2_temporal > 0, 1, NA)

  # pid
  x1_x2_temporal_pid <- terra::patches(x1_x2_temporal_null)

  # table
  table_x1_x2 <- terra::crosstab(c(x1_x2_temporal_pid, x1_x2_temporal), long = TRUE)

  rgrass::execGRASS(cmd = "r.stats", flags = c("c", "n", "overwrite"), input = "", output = "x1_x2_temporal_pid.csv", separator = ",")
