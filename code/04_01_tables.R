...
## roads railways ----
area_trimmed <- readr::read_csv("02_results/00_roads_railways_area.csv",
...
length_trimmed <- length_roads + length_railways
length_trimmed
...
area_antropic <- dplyr::bind_rows(area_trimmed,
                                  area_protected_areas,
                                  area_indigenous_territories)
area_antropic
...
for(i in files_fid_area){
  ...
  scenario <- ifelse(title_fid_area_letters == "Forest", "Forest vegetation (not trimmed)",
                     ifelse(title_fid_area_letters == "Forest Trimmed", "Forest vegetation (trimmed)",
                            ifelse(title_fid_area_letters == "Natural", "Natural vegetation (not trimmed)",
                                   ifelse(title_fid_area_letters == "Natural Trimmed", "Natural vegetation (trimmed)", NA))))
  ...
  data_fid_area_i_class <- data_fid_area_i %>%
    ...
    dplyr::bind_rows(tibble::tibble(values = 0, area = 0, n = 0, area_ha = 0, class = "500,000-1,000,000")) %>%
  ...
  data_fid_area_i_class_resume <- data_fid_area_i_class %>%
    ...
    dplyr::summarise(class_area = sum(area_ha),
                     class_n = n()) %>%
    dplyr::mutate(class_n = ifelse(class_area == 0, 0, class_n)) %>%
  ...
  data_fid_area_resume_i <- tibble::tibble(year = year,
                                           ...
                                           average_area_ha = round(mean(data_fid_area_i$area_ha), 2),
                                           sd_area_ha = round(sd(data_fid_area_i$area_ha), 2),
                                           cv_area_ha = round(sd_area_ha/average_area_ha, 2),
...
data_fid_area_resume_1_50ha <- data_fid_area %>%
  dplyr::filter(scenario %in% c("Forest vegetation (trimmed)", "Natural vegetation (trimmed)"),
...
max(data_fid_area_resume_1_50ha$class_n_per_sum) - min(data_fid_area_resume_1_50ha$class_n_per_sum)
dplyr::filter(data_fid_area_resume_1_50ha, scenario == "Forest vegetation (trimmed)")
dplyr::filter(data_fid_area_resume_1_50ha, scenario == "Natural vegetation (trimmed)")
...
dplyr::filter(data_fid_area_resume_50_25kha, scenario == "Forest vegetation (trimmed)")
dplyr::filter(data_fid_area_resume_50_25kha, scenario == "Natural vegetation (trimmed)")
...
dplyr::filter(data_fid_area_resume_25kha, scenario == "Forest vegetation (trimmed)")
dplyr::filter(data_fid_area_resume_25kha, scenario == "Natural vegetation (trimmed)")
...
data_fid_area_resume %>%
  ...
  dplyr::arrange(scenario) %>%
  dplyr::select(year, scenario, n_patches, average_area_ha)
...
for(i in unique(data_fid_area_resume$scenario)){
  ...
  data_fid_area_resume_i <- data_fid_area_resume %>%
    dplyr::select(year, scenario, n_patches, average_area_ha, max_area_ha) %>%
    dplyr::filter(scenario == i)

  data_fid_area_resume_pct_change_i_2005_1986_pct <- data_fid_area_resume_i %>%
    dplyr::filter(year %in% c(1986, 2005)) %>%
    dplyr::arrange(year, .by_group = TRUE) %>%
    dplyr::mutate(pct_change_n_patches = round((n_patches/lag(n_patches) - 1) * 100, 2),
                  pct_change_average_area_ha = round((average_area_ha/lag(average_area_ha) - 1) * 100, 2),
                  pct_change_max_area_ha = round((max_area_ha/lag(max_area_ha) - 1) * 100, 2)) %>%
    tidyr::drop_na() %>%
    dplyr::select(pct_change_n_patches:pct_change_max_area_ha)

  data_fid_area_resume_pct_change_i_2005_1986 <- data_fid_area_resume_i %>%
    dplyr::filter(year %in% c(1986, 2005)) %>%
    dplyr::arrange(year, .by_group = TRUE) %>%
    dplyr::summarize(across(everything(), ~ paste(.x, collapse = '-'))) %>%
    dplyr::mutate(year = "2005-1986",
                  scenario = i) %>%
    dplyr::bind_cols(data_fid_area_resume_pct_change_i_2005_1986_pct)

  data_fid_area_resume_pct_change_i_2020_1986_pct <- data_fid_area_resume_i %>%
    dplyr::filter(year %in% c(2020, 1986)) %>%
    dplyr::arrange(year, .by_group = TRUE) %>%
    dplyr::mutate(pct_change_n_patches = round((n_patches/lag(n_patches) - 1) * 100, 2),
                  pct_change_average_area_ha = round((average_area_ha/lag(average_area_ha) - 1) * 100, 2),
                  pct_change_max_area_ha = round((max_area_ha/lag(max_area_ha) - 1) * 100, 2)) %>%
  ...
  data_fid_area_resume_pct_change_i_2020_1986 <- data_fid_area_resume_i %>%
    ...
    dplyr::mutate(year = "2020-1986",
                  scenario = i) %>%
    dplyr::bind_cols(data_fid_area_resume_pct_change_i_2020_1986_pct)

  data_fid_area_resume_pct_change_i_2020_2005_pct <- data_fid_area_resume_i %>%
    dplyr::filter(year %in% c(2020, 2005)) %>%
    dplyr::arrange(year, .by_group = TRUE) %>%
    dplyr::mutate(pct_change_n_patches = round((n_patches/lag(n_patches) - 1) * 100, 2),
                  pct_change_average_area_ha = round((average_area_ha/lag(average_area_ha) - 1) * 100, 2),
                  pct_change_max_area_ha = round((max_area_ha/lag(max_area_ha) - 1) * 100, 2)) %>%
  ...
  data_fid_area_resume_pct_change_i_2020_2005 <- data_fid_area_resume_i %>%
    ...
    dplyr::mutate(year = "2020-2005",
                  scenario = i) %>%
    dplyr::bind_cols(data_fid_area_resume_pct_change_i_2020_2005_pct)
...
data_fid_area_resume_pct_change_filter <- data_fid_area_resume_pct_change %>%
  dplyr::filter(year %in% c("2005-1986", "2020-2005"),
                scenario %in% c("Forest vegetation (trimmed)", "Natural vegetation (trimmed)"))
data_fid_area_resume_pct_change_filter
...
# import roads
forest_1986_2005_trimmed <- readr::read_csv("02_results/01_mapbiomas_brazil_af_trinacional_1986_af_lim_forest_trimmed_mapbiomas_brazil_af_trinacional_2005_af_lim_forest_trimmed_temporal_patches_metrics.csv") %>%
  dplyr::mutate(period = "1986_2005",
                scenario = "forest_trimmed", .before = 1)
forest_1986_2005_trimmed

forest_1986_2020_trimmed <- readr::read_csv("02_results/01_mapbiomas_brazil_af_trinacional_1986_af_lim_forest_trimmed_mapbiomas_brazil_af_trinacional_2020_af_lim_forest_trimmed_temporal_patches_metrics.csv") %>%
  dplyr::mutate(period = "1986_2020",
                scenario = "forest_trimmed", .before = 1)
forest_1986_2020_trimmed

forest_2005_2020_trimmed <- readr::read_csv("02_results/01_mapbiomas_brazil_af_trinacional_2005_af_lim_forest_trimmed_mapbiomas_brazil_af_trinacional_2020_af_lim_forest_trimmed_temporal_patches_metrics.csv") %>%
  dplyr::mutate(period = "2005_2020",
                scenario = "forest_trimmed", .before = 1)
forest_2005_2020_trimmed

natural_1986_2005_trimmed <- readr::read_csv("02_results/01_mapbiomas_brazil_af_trinacional_1986_af_lim_natural_trimmed_mapbiomas_brazil_af_trinacional_2005_af_lim_natural_trimmed_temporal_patches_metrics.csv") %>%
  dplyr::mutate(period = "1986_2005",
                scenario = "natural_trimmed", .before = 1)
natural_1986_2005_trimmed

natural_1986_2020_trimmed <- readr::read_csv("02_results/01_mapbiomas_brazil_af_trinacional_1986_af_lim_natural_trimmed_mapbiomas_brazil_af_trinacional_2020_af_lim_natural_trimmed_temporal_patches_metrics.csv") %>%
  dplyr::mutate(period = "1986_2020",
                scenario = "natural_trimmed", .before = 1)
natural_1986_2020_trimmed

natural_2005_2020_trimmed <- readr::read_csv("02_results/01_mapbiomas_brazil_af_trinacional_2005_af_lim_natural_trimmed_mapbiomas_brazil_af_trinacional_2020_af_lim_natural_trimmed_temporal_patches_metrics.csv") %>%
  dplyr::mutate(period = "2005_2020",
                scenario = "natural_trimmed", .before = 1)
natural_2005_2020_trimmed
...
data_temporal <- dplyr::bind_rows(forest_1986_2005, forest_1986_2020, forest_2005_2020,
                                  ...
                                  forest_1986_2005_trimmed, forest_1986_2020_trimmed, forest_2005_2020_trimmed,
                                  natural_1986_2005_trimmed, natural_1986_2020_trimmed, natural_2005_2020_trimmed)
data_temporal
...
for(i in files_habitat_cover){
  ...
  scenario <- ifelse(length(names_habitat_cover) > 12,
                     paste0(names_habitat_cover[, 9], "_", names_habitat_cover[, 11]),
...
data_habitat_cover_resume_total <- dplyr::bind_rows(data_habitat_cover, data_habitat_cover_resume) %>%
  ...
  dplyr::mutate(scenario = dplyr::case_when(
    ...
    scenario == "forest_trimmed" ~ "Forest vegetation (trimmed)",
    scenario == "natural" ~ "Natural vegetation (not trimmed)",
    scenario == "natural_trimmed" ~ "Natural vegetation (trimmed)"))
data_habitat_cover_resume_total

data_habitat_cover_resume_total_filter <- data_habitat_cover_resume_total %>%
  dplyr::filter(year %in% c(1986, 2020),
                scenario %in% c("Forest vegetation (trimmed)", "Natural vegetation (trimmed)"),
                classes %in% c(1, 10)) %>%
  dplyr::mutate(area_ha = round(area_ha/1e6, 1))
data_habitat_cover_resume_total_filter
...
for(i in files_habitat_cover_br){
  ...
  scenario_br <- ifelse(length(names_habitat_cover_br) > 13,
                        paste0(names_habitat_cover_br[, 9], "_", names_habitat_cover_br[, 11]),
...
data_habitat_cover_resume_total_br_filter <- data_habitat_cover_resume_total_br %>%
  dplyr::filter(year %in% c(1986, 2005,2020),
                scenario %in% c("Forest vegetation (trimmed)", "Natural vegetation (trimmed)"),
                classes %in% c(1, 10)) %>%
  dplyr::mutate(area_ha = round(area_ha/1e6, 1))
data_habitat_cover_resume_total_br_filter
...
for(i in files_habitat_cover_ar){
  ...
  scenario_ar <- ifelse(length(names_habitat_cover_ar) > 13,
                        paste0(names_habitat_cover_ar[, 9], "_", names_habitat_cover_ar[, 11]),
...
data_habitat_cover_resume_total_ar_filter <- data_habitat_cover_resume_total_ar %>%
  dplyr::filter(year %in% c(1986, 2015, 2020),
                scenario %in% c("Forest vegetation (trimmed)", "Natural vegetation (trimmed)"),
                classes %in% c(1, 10)) %>%
  dplyr::mutate(area_ha = round(area_ha/1e6, 2))
data_habitat_cover_resume_total_ar_filter
...
for(i in files_habitat_cover_py){
  ...
  scenario_py <- ifelse(length(names_habitat_cover_py) > 13,
                        paste0(names_habitat_cover_py[, 9], "_", names_habitat_cover_py[, 11]),
...
data_habitat_cover_resume_total_py_filter <- data_habitat_cover_resume_total_py %>%
  dplyr::filter(year %in% c(1986, 2005, 2020),
                scenario %in% c("Forest vegetation (trimmed)", "Natural vegetation (trimmed)"),
                classes %in% c(1, 10)) %>%
  dplyr::mutate(area_ha = round(area_ha/1e6, 2))
data_habitat_cover_resume_total_py_filter
...
for(i in files_limits_habitat_cover){
  ...
  scenario <- ifelse(length(names_habitat_cover) > 12,
                     paste0(names_habitat_cover[, 10], "_", names_habitat_cover[, 11]),
  ...
  data_limits_habitat_cover_i <- readr::read_csv(i, col_names = c("values", "area", "n", "p"), col_types = cols()) %>%
    ...
    dplyr::mutate(percentage = as.numeric(stringr::str_replace(p, "%", "")))
...
for(i in files_limits_area){
  ...
  scenario <- ifelse(length(name_area) > 12,
                     paste0(name_area[, 10], "_", name_area[, 11]),
                     name_area[, 10])
...
data_limits_habitat_cover_area %>%
  dplyr::filter(grepl("Forest", scenario)) %>%
  dplyr::filter(percentage == min(percentage)) %>%
  dplyr::select(1:2, total_area_ha, percentage) %>%
  dplyr::mutate(total_area_ha = total_area_ha/1e6)
...
data_limits_habitat_cover_area %>%
  dplyr::filter(grepl("Natural", scenario)) %>%
  dplyr::filter(percentage == min(percentage)) %>%
  dplyr::select(1:2, total_area_ha, percentage) %>%
  dplyr::mutate(total_area_ha = total_area_ha/1e6)
...
data_core_edge_area_30m_forest <- data_core_edge_area %>%
  dplyr::filter(scenario == "forest trimmed", edge_dist == 30) %>%
  dplyr::group_by(year, scenario)
data_core_edge_area_30m_forest

data_core_edge_area_30m_natural <- data_core_edge_area %>%
  dplyr::filter(scenario == "natural trimmed", edge_dist == 30)
data_core_edge_area_30m_natural

data_core_edge_area_90m_forest <- data_core_edge_area %>%
  dplyr::filter(scenario == "forest trimmed", edge_dist == 90)
data_core_edge_area_90m_forest

data_core_edge_area_90m_natural <- data_core_edge_area %>%
  dplyr::filter(scenario == "natural trimmed", edge_dist == 90)
data_core_edge_area_90m_natural

data_core_edge_area_240m_forest <- data_core_edge_area %>%
  dplyr::filter(scenario == "forest trimmed", edge_dist == 240)
data_core_edge_area_240m_forest

data_core_edge_area_240m_natural <- data_core_edge_area %>%
  dplyr::filter(scenario == "natural trimmed", edge_dist == 240)
data_core_edge_area_240m_natural

data_core_edge_area_500m_forest <- data_core_edge_area %>%
  dplyr::filter(scenario == "forest trimmed", edge_dist >= 500) %>%
  group_by(scenario, year) %>%
  summarise(per_sum = sum(per))
data_core_edge_area_500m_forest

data_core_edge_area_500m_natural <- data_core_edge_area %>%
  dplyr::filter(scenario == "natural trimmed", edge_dist > 500) %>%
  group_by(scenario, year) %>%
  summarise(per_sum = sum(per))
data_core_edge_area_500m_natural

data_core_edge_area %>%
  dplyr::filter(scenario == "forest trimmed") %>%
  dplyr::pull(edge_dist) %>%
  max()

data_core_edge_area %>%
  dplyr::filter(scenario == "natural trimmed") %>%
  dplyr::pull(edge_dist) %>%
  max()

# export
data_core_edge_area_scenario <- data_core_edge_area %>%
  dplyr::mutate(scenario = dplyr::case_when(
    scenario == "forest" ~ "Forest vegetation (not trimmed)",
    scenario == "forest trimmed" ~ "Forest vegetation (trimmed)",
    scenario == "natural" ~ "Natural vegetation (not trimmed)",
    scenario == "natural trimmed" ~ "Natural vegetation (trimmed)"))
data_core_edge_area_scenario

readr::write_csv(data_core_edge_area_scenario, "03_tables/data_fig05.csv")
...
# import data
scenarios <- c("forest", "natural", "forest_trimmed", "natural_trimmed")
...
## summary ----
# functional connectivity 0
data_connectivity_summary_0 <- data_connectivity_summary %>%
  dplyr::filter(year %in% c(1986, 2005, 2020),
                scenario %in% c("Forest vegetation (trimmed)",
                                "Natural vegetation (trimmed)"),
                gap_crossing == 0) %>%
  dplyr::arrange(scenario)
data_connectivity_summary_0

(data_connectivity_summary_0[3, 4]-data_connectivity_summary_0[1, 4])/data_connectivity_summary_0[1, 4]*100
(data_connectivity_summary_0[6, 4]-data_connectivity_summary_0[4, 4])/data_connectivity_summary_0[4, 4]*100

data_connectivity_summary_0 <- data_connectivity_summary %>%
  dplyr::filter(scenario %in% c("Forest vegetation (trimmed)",
                                "Natural vegetation (trimmed)"),
                gap_crossing == 0) %>%
  dplyr::group_by(scenario) %>%
  dplyr::summarise(high_cluster_mean = mean(high_cluster))
data_connectivity_summary_0

# functional connectivity 60
data_connectivity_summary_60 <- data_connectivity_summary %>%
  dplyr::filter(year %in% c(1986, 2005, 2020),
                scenario %in% c("Forest vegetation (trimmed)",
                                "Natural vegetation (trimmed)"),
                gap_crossing == "0060") %>%
  dplyr::arrange(scenario)
data_connectivity_summary_60

(data_connectivity_summary_60[3, 4]-data_connectivity_summary_60[1, 4])/data_connectivity_summary_60[1, 4]*100
(data_connectivity_summary_60[6, 4]-data_connectivity_summary_60[4, 4])/data_connectivity_summary_60[4, 4]*100
...
# functional connectivity 120
data_connectivity_summary_120 <- data_connectivity_summary %>%
  dplyr::filter(scenario %in% c("Forest vegetation (trimmed)",
                                "Natural vegetation (trimmed)"),
                gap_crossing == "0120") %>%
  dplyr::arrange(scenario)
data_connectivity_summary_120

# functional connectivity 180
data_connectivity_summary_180 <- data_connectivity_summary %>%
  dplyr::filter(year %in% c(1986, 2005, 2020),
                scenario %in% c("Forest vegetation (trimmed)",
                                "Natural vegetation (trimmed)"),
                gap_crossing == "0180") %>%
  dplyr::arrange(scenario)
data_connectivity_summary_180

# functional connectivity 240
data_connectivity_summary_240 <- data_connectivity_summary %>%
  dplyr::filter(year %in% c(1986, 2005, 2020),
                scenario %in% c("Forest vegetation (trimmed)",
                                "Natural vegetation (trimmed)"),
                gap_crossing == "0240") %>%
  dplyr::arrange(scenario)
data_connectivity_summary_240

# functional connectivity 300
data_connectivity_summary_300 <- data_connectivity_summary %>%
  dplyr::filter(year %in% c(1986, 2005, 2020),
                scenario %in% c("Forest vegetation (trimmed)",
                                "Natural vegetation (trimmed)"),
                gap_crossing == "0300") %>%
  dplyr::arrange(scenario)
data_connectivity_summary_300

# functional connectivity 600
data_connectivity_summary_600 <- data_connectivity_summary %>%
  dplyr::filter(year %in% c(1986, 2005, 2020),
                scenario %in% c("Forest vegetation (trimmed)",
                                "Natural vegetation (trimmed)"),
                gap_crossing == "0600") %>%
  dplyr::arrange(scenario)
data_connectivity_summary_600
...
# functional connectivity 900
data_connectivity_summary_900 <- data_connectivity_summary %>%
  dplyr::filter(year %in% c(1986, 2005, 2020),
                scenario %in% c("Forest vegetation (trimmed)",
                                "Natural vegetation (trimmed)"),
                gap_crossing == "0600") %>%
  dplyr::arrange(scenario)
data_connectivity_summary_900

# functional connectivity 1200
data_connectivity_summary_1200 <- data_connectivity_summary %>%
  dplyr::filter(year %in% c(1986, 2005, 2020),
                scenario %in% c("Forest vegetation (trimmed)",
                                "Natural vegetation (trimmed)"),
                gap_crossing == "0600") %>%
  dplyr::arrange(scenario)
data_connectivity_summary_1200

# functional connectivity 1500
data_connectivity_summary_1500 <- data_connectivity_summary %>%
  dplyr::filter(year %in% c(1986, 2005, 2020),
                scenario %in% c("Forest vegetation (trimmed)",
                                "Natural vegetation (trimmed)"),
                gap_crossing == "0600") %>%
  dplyr::arrange(scenario)
data_connectivity_summary_1500


data_connectivity_summary_60_600 <- data_connectivity_summary %>%
  dplyr::filter(scenario %in% c("Forest vegetation (trimmed)",
                                "Natural vegetation (trimmed)"),
                !gap_crossing %in% c("0", "0900", "1200", "1500")) %>%
  dplyr::arrange(scenario)
range(data_connectivity_summary_60_600$high_cluster)
...
for(i in files_isolation){
  ...
  data_isolation_mean_i <- readr::read_csv(i) %>%
    dplyr::mutate(scenario = ifelse(length(names_isolation) == 12,
                                    paste0(names_isolation[, 9], "_", names_isolation[, 10]),
                                    names_isolation[, 9]),
                  year = names_isolation[, 6],
                  area = ifelse(length(names_isolation) == 12,
                                stringr::str_extract(names_isolation[, 12], pattern = "[0-9]+"),
...
data_isolation_resume_forest_smaller <- data_isolation_resume %>%
  dplyr::filter(!area == "0000",
                scenario == "Forest vegetation (trimmed)") %>%
  dplyr::arrange(scenario, year)
range(data_isolation_resume_forest_smaller$mean)

data_isolation_resume_natural_smaller <- data_isolation_resume %>%
  dplyr::filter(!area == "0000",
                scenario == "Natural vegetation (trimmed)") %>%
  dplyr::arrange(scenario, year)
range(data_isolation_resume_natural_smaller$mean)
...
data_isolation_resume_100_forest <- data_isolation_resume %>%
  dplyr::filter(area == "0100",
                scenario %in% c("Forest vegetation (trimmed)")) %>%
  dplyr::arrange(scenario, year)
data_isolation_resume_100_forest

data_isolation_resume_100_natural <- data_isolation_resume %>%
  dplyr::filter(area == "0100",
                scenario %in% c("Natural vegetation (trimmed)")) %>%
  dplyr::arrange(scenario, year)
data_isolation_resume_100_natural

data_isolation_resume_150_forest <- data_isolation_resume %>%
  dplyr::filter(area == "0150",
                scenario %in% c("Forest vegetation (trimmed)")) %>%
  dplyr::arrange(scenario, year)
data_isolation_resume_150_forest

data_isolation_resume_150_natural <- data_isolation_resume %>%
  dplyr::filter(area == "0150",
                scenario %in% c("Natural vegetation (trimmed)")) %>%
  dplyr::arrange(scenario, year)
data_isolation_resume_150_natural

data_isolation_resume_200_forest <- data_isolation_resume %>%
  dplyr::filter(area == "0200",
                scenario %in% c("Forest vegetation (trimmed)")) %>%
  dplyr::arrange(scenario, year)
data_isolation_resume_200_forest

data_isolation_resume_200_natural <- data_isolation_resume %>%
  dplyr::filter(area == "0200",
                scenario %in% c("Natural vegetation (trimmed)")) %>%
  dplyr::arrange(scenario, year)
data_isolation_resume_200_natural

data_isolation_resume_250_forest <- data_isolation_resume %>%
  dplyr::filter(area == "0250",
                scenario %in% c("Forest vegetation (trimmed)")) %>%
  dplyr::arrange(scenario, year)
data_isolation_resume_250_forest

data_isolation_resume_250_natural <- data_isolation_resume %>%
  dplyr::filter(area == "0250",
                scenario %in% c("Natural vegetation (trimmed)")) %>%
  dplyr::arrange(scenario, year)
data_isolation_resume_250_natural

data_isolation_resume_350_forest <- data_isolation_resume %>%
  dplyr::filter(area == "0350",
                scenario %in% c("Forest vegetation (trimmed)")) %>%
  dplyr::arrange(scenario, year)
data_isolation_resume_350_forest

data_isolation_resume_350_natural <- data_isolation_resume %>%
  dplyr::filter(area == "0350",
                scenario %in% c("Natural vegetation (trimmed)")) %>%
  dplyr::arrange(scenario, year)
data_isolation_resume_350_natural

data_isolation_resume_500_forest <- data_isolation_resume %>%
  dplyr::filter(area == "0500",
                scenario %in% c("Forest vegetation (trimmed)")) %>%
  dplyr::arrange(scenario, year)
data_isolation_resume_500_forest

data_isolation_resume_500_natural <- data_isolation_resume %>%
  dplyr::filter(area == "0500",
                scenario %in% c("Natural vegetation (trimmed)")) %>%
  dplyr::arrange(scenario, year)
data_isolation_resume_500_natural

data_isolation_resume_1000_forest <- data_isolation_resume %>%
  dplyr::filter(area == "1000",
                scenario %in% c("Forest vegetation (trimmed)")) %>%
  dplyr::arrange(scenario, year)
data_isolation_resume_1000_forest

data_isolation_resume_1000_natural <- data_isolation_resume %>%
  dplyr::filter(area == "1000",
                scenario %in% c("Natural vegetation (trimmed)")) %>%
  dplyr::arrange(scenario, year)
data_isolation_resume_1000_natural

sort((data_isolation_resume_050_forest$mean - data_isolation_resume_000_forest$mean)/data_isolation_resume_000_forest$mean * 100)
sort((data_isolation_resume_050_natural$mean - data_isolation_resume_000_natural$mean)/data_isolation_resume_000_natural$mean * 100)
...
sort((data_isolation_resume_050_natural$mean - data_isolation_resume_050_forest$mean)/data_isolation_resume_050_forest$mean * 100)
sort((data_isolation_resume_100_natural$mean - data_isolation_resume_100_forest$mean)/data_isolation_resume_100_forest$mean * 100)
sort((data_isolation_resume_150_natural$mean - data_isolation_resume_150_forest$mean)/data_isolation_resume_150_forest$mean * 100)
sort((data_isolation_resume_200_natural$mean - data_isolation_resume_200_forest$mean)/data_isolation_resume_200_forest$mean * 100)
sort((data_isolation_resume_250_natural$mean - data_isolation_resume_250_forest$mean)/data_isolation_resume_250_forest$mean * 100)
sort((data_isolation_resume_350_natural$mean - data_isolation_resume_350_forest$mean)/data_isolation_resume_350_forest$mean * 100)
sort((data_isolation_resume_500_natural$mean - data_isolation_resume_500_forest$mean)/data_isolation_resume_500_forest$mean * 100)
sort((data_isolation_resume_1000_natural$mean - data_isolation_resume_1000_forest$mean)/data_isolation_resume_1000_forest$mean * 100)
...
### area ----
area_2020_veg_trimmed <- data_fid_area_resume %>%
  dplyr::filter(year == "2020",
                scenario %in% c("Forest vegetation (trimmed)",
                                "Natural vegetation (trimmed)")) %>%
  dplyr::pull(total_area_ha)
area_2020_veg_trimmed
...
area_protected_areas <- readr::read_csv("02_results/00_protected_areas_area.csv",
  ...
  dplyr::summarise(area_ha = sum(area)/1e4) %>%
  dplyr::filter(anthropic == 1) %>%
  dplyr::mutate(area_ha_m = area_ha/1e6,
                percentage_vancine_etal_inprep = area_ha/area_af_lim$area_ha*100,
                percentage_fv = area_ha/area_2020_veg_trimmed[1]*100,
                percentage_nv = area_ha/area_2020_veg_trimmed[2]*100,
                anthropic = "Protected areas")
area_protected_areas
...
for(i in files_protected_area){
  ...
  data_protected_area_i <- readr::read_csv(i, col_names = c("vegetation", "dist_pa", "area", "n"), col_types = readr::cols()) %>%
    ...
    dplyr::mutate(n_total_per = round(n_total/sum(n_total)*100, 2)) %>%
    dplyr::mutate(scenario = ifelse(length(names_protected_area) == 12,
                                    paste0(names_protected_area[, 9], "_", names_protected_area[, 10]),
...
# analysis
data_protected_area %>%
  dplyr::filter(scenario == "forest_trimmed",
                class == "PA") %>%
...
data_protected_area %>%
  dplyr::filter(scenario == "natural_trimmed",
                class == "PA") %>%
  dplyr::pull(area_ha_total) %>%
  sum()/1e6
...
# not trimmed
data_protected_area_natural_classes <- readr::read_csv("02_results/06_mapbiomas_brazil_af_trinacional_2020_af_lim_natural_protected_areas_classes.csv",
...
# trimmed
data_protected_area_natural_classes_roads <- readr::read_csv("02_results/06_mapbiomas_brazil_af_trinacional_2020_af_lim_natural_trimmed_protected_areas_classes.csv",
                                                             col_names = c("values", "pa", "area", "n"), col_types = readr::cols()) %>%
  dplyr::mutate(area_ha = area/1e4,
                per_total = round(area_ha/sum(area_ha) * 100, 3)) %>%
  dplyr::select(1, 2, 5, 6)
...
for(i in files_indigenous_territories){
  ...
  data_indigenous_territories_i <- readr::read_csv(i, col_names = c("vegetation", "dist_pa", "area", "n"), col_types = readr::cols()) %>%
    ...
    dplyr::mutate(scenario = ifelse(length(names_indigenous_territories) == 12,
                                    paste0(names_indigenous_territories[, 9], "_", names_indigenous_territories[, 10]),
...
# not trimmed
data_indigenous_territories_natural_classes <- readr::read_csv("02_results/06_mapbiomas_brazil_af_trinacional_2020_af_lim_natural_indigenous_territories_classes.csv",
                                                               col_names = c("values", "pa", "area", "n"), col_types = readr::cols()) %>%
  dplyr::mutate(area_ha = area/1e4,
                per_total = round(area_ha/sum(area_ha) * 100, 3)) %>%
  dplyr::select(1, 2, 5, 6)
...
# trimmed
data_indigenous_territories_natural_classes_roads <- readr::read_csv("02_results/06_mapbiomas_brazil_af_trinacional_2020_af_lim_natural_trimmed_indigenous_territories_classes.csv",
                                                                     col_names = c("values", "pa", "area", "n"), col_types = readr::cols()) %>%
  dplyr::mutate(area_ha = area/1e4,
                per_total = round(area_ha/sum(area_ha) * 100, 3)) %>%
  dplyr::select(1, 2, 5, 6)
...
## table s01 ----
area_limits %>%
  dplyr::mutate(area_ha = round(area_ha),
                percentage_vancine_etal_inprep = round(percentage_vancine_etal_inprep, 2)) %>%
  dplyr::mutate(area_ha = paste0(stringr::str_sub(area_ha, start = -9, end = -7), ",",
                                 stringr::str_sub(area_ha, start = -6, end = -4), ",",
                                 stringr::str_sub(area_ha, start = -3))) %>%
  writexl::write_xlsx("03_tables/table_s01.xslx")

## table s04 ----
data_fid_area_resume %>%
  dplyr::select(year, scenario, percentage, n_patches, total_area_ha,
                average_area_ha, sd_area_ha, median_area_ha, max_area_ha) %>%
  dplyr::mutate(n_patches = paste0(stringr::str_sub(n_patches, start = -9, end = -7), ",",
                                   stringr::str_sub(n_patches, start = -6, end = -4), ",",
                                   stringr::str_sub(n_patches, start = -3)),
                total_area_ha = paste0(stringr::str_sub(total_area_ha, start = -9, end = -7), ",",
                                       stringr::str_sub(total_area_ha, start = -6, end = -4), ",",
                                       stringr::str_sub(total_area_ha, start = -3)),
                sd_area_ha = ifelse(sd_area_ha < 1e3,
                                    sd_area_ha,
                                    paste0(stringr::str_sub(sd_area_ha, start = -9, end = -7), ",",
                                           stringr::str_sub(sd_area_ha, start = -6, end = -4),
                                           stringr::str_sub(sd_area_ha, start = -3))),
                max_area_ha = ifelse(max_area_ha < 1e6,
                                     paste0(stringr::str_sub(max_area_ha, start = -6, end = -4), ",",
                                            stringr::str_sub(max_area_ha, start = -3)),
                                     paste0(stringr::str_sub(max_area_ha, start = -9, end = -7), ",",
                                            stringr::str_sub(max_area_ha, start = -6, end = -4), ",",
                                            stringr::str_sub(max_area_ha, start = -3)))) %>%
  writexl::write_xlsx("03_tables/table_s04.xlsx")
...
data_temporal_wide %>%
  ...
  dplyr::mutate(period = case_when(period == "1986_2005" ~ "1986-2005",
                ...
                scenario = case_when(scenario == "forest" ~ "FV not trimmed",
                                     scenario == "forest_trimmed" ~ "FV trimmed",
                                     scenario == "natural" ~ "NV not trimmed",
                                     scenario == "natural_trimmed" ~ "NV trimmed"),
  ...
  dplyr::mutate(value_balance_area_total = ifelse(abs(value_balance_area_total) > 1e6,
                                                  paste0(stringr::str_sub(value_balance_area_total, start = -9, end = -7), ",",
                                                         stringr::str_sub(value_balance_area_total, start = -6, end = -4), ",",
                                                         stringr::str_sub(value_balance_area_total, start = -3)),
                                                  paste0(stringr::str_sub(value_balance_area_total, start = -6, end = -4), ",",
                                                         stringr::str_sub(value_balance_area_total, start = -3))),
                value_balance_patches_number = paste0(stringr::str_sub(value_balance_patches_number, start = -6, end = -4), ",",
                                                      stringr::str_sub(value_balance_patches_number, start = -3))) %>%
  writexl::write_xlsx("03_tables/table_s05.xlsx")

## table s06 ----
data_limits_habitat_cover_area %>%
  dplyr::select(limit, scenario, percentage, n_patches, total_area_ha,
  ...
  dplyr::mutate(n_patches = paste0(stringr::str_sub(n_patches, start = -9, end = -7), ",",
                ...
                max_area_ha = ifelse(max_area_ha < 1e6,
                                     ...
                                     paste0(stringr::str_sub(max_area_ha, start = -9, end = -7), ",",
                                            stringr::str_sub(max_area_ha, start = -6, end = -4), ",",
                                            stringr::str_sub(max_area_ha, start = -3)))) %>%
  writexl::write_xlsx("03_tables/table_s06.xlsx")
